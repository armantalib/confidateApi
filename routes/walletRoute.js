...
router.put('/add', async (req, res) => {
  ...
  //   balance: balance,
  //   type: "deposit",
  //   description: balance+req.user.lang=='spanish'?lang["dep"]:lang["dep"],
  // });
  ...
  if (!wallet) {
    ...
    return res.send({ success: true, message: req.user.lang == 'spanish' ? lang["balcadd"] : lang["balcadd"], wallet: wallets });
  }
...
router.put('/withdraw', async (req, res) => {
  ...
  const transaction = new Transaction({
    ...
    type: "withdraw",
    status: "pending",
    description: balance + req.user.lang == 'spanish' ? lang2["withdraw"] : lang["withdraw"],
...
router.get('/admin/payment/:id', admin, async (req, res) => {
  console.log("Run is");
  let query = {};
  ...
  try {
    const data = await Wallet.find(query).populate("user")
      .sort({ _id: -1 })
      .skip(skip)
      .limit(pageSize).lean();

    const totalCount = await Wallet.find(query);
