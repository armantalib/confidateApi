...
exports.searchGigs = async (req, res) => {
  ...
  if (type=='search'&&req.params.search==undefined) {
  ...
  switch (type) {
    case 'related':
      if (user) {
        query.category = user.profession
      }
      break;
    case 'promoted':
      if (user) {
...
exports.detailsGigs = async (req, res) => {
  ...
  if (req?.user?._id) {
    ...
    likes = Array.isArray(gig.likes) && gig.likes.some(like => like.user.toString() === req.user._id.toString());
  }
