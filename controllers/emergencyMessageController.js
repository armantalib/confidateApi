...
exports.create = async (req, res) => {
  try {
    const { message,message_t} = req.body;
    const userId = req.user._id;
    const createRecord = new EmergencyMessages({
      user_id: userId,
      message,
      message_t
    });
    ...
    res.status(201).json({ success: true, message: 'Emergency Message Created Successfully',emergencyMessage:createRecord });
  } catch (error) {
